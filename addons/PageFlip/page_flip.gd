...
func _get_page_index_for_spread(spread_idx: int, is_left: bool) -> int:
	...
	if is_left:
		...
		if base >= _runtime_pages.size(): return -999
		return base
	else:
		# Página derecha: base + 1 (impar)
		if base + 1 >= _runtime_pages.size(): return -999
		return base + 1
...
func _update_slot_content(slot: SubViewport, content_index: int) -> void:
	...
	if content_index == -999: return  # Slot vacío / invisible
...
# ==============================================================================
func next_page():
	if is_animating or current_spread >= total_spreads - 1: return
	_start_animation(true)


func prev_page():
	if is_animating or current_spread <= 0: return
...
func go_to_page(page_num: int = 1) -> void:
	...
	if target_spread == current_spread: return
	_is_jumping = true
...
func _start_animation(forward: bool) -> void:
	...
	if _is_jumping: target_spread_idx = _jump_target_spread
	...
	if dynamic_poly.material is ShaderMaterial:
		...
		if target_spread_val == null or target_spread_val == 0.0: target_spread_val = 0.5
...
func _on_animation_finished(_anim_name: String):
	...
	if _is_jumping:
		...
		if going_forward: current_spread = min(current_spread + 1, total_spreads - 1)
		else: current_spread = max(current_spread - 1, 0)
